...
namespace CalValEX.Tiles.Statues
{
    public class PlagueFrogStatuePlaced : ModTile
    {
        ...
        public override void HitWire(int i, int j)
        {
            ...
            if (Wiring.CheckMech(x, y, 30) && NPC.MechSpawn((float)spawnX, (float)spawnY, (NPCType<PlagueFrog>())))
            {
                npcIndex = NPC.NewNPC(new Terraria.DataStructures.EntitySource_SpawnNPC(),spawnX, spawnY - 12, (NPCType<PlagueFrog>()));
            }
            if (npcIndex >= 0)
            {
