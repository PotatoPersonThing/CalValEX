...
namespace CalValEX.Tiles.Statues
{
    public class IsopodStatuePlaced : ModTile
    {
        ...
        public override void HitWire(int i, int j)
        {
            ...
            if (Wiring.CheckMech(x, y, 30) && NPC.MechSpawn((float)spawnX, (float)spawnY, (NPCType<Isopod>())))
            {
                npcIndex = NPC.NewNPC(new Terraria.DataStructures.EntitySource_SpawnNPC(),spawnX, spawnY - 12, (NPCType<Isopod>()));
            }
            if (npcIndex >= 0)
            {
