    ...
    public class Drops : GlobalNPC
    {
        ...
        public override void NPCLoot(NPC npc)
        {
            ...
            if (!CalValEXConfig.Instance.DisableVanityDrops)
            {
                ...
                if (npc.type == calamityMod.NPCType("AstralProbe"))
                {
                    ChanceDropItem(npc,
                        Main.rand.NextBool()
                            ...
                            : ModContent.ItemType<AstralOldYellow>(), 0.001f); //0.1% for either
                }
                ...
                {
                    ConditionalChanceDropItem(npc, ModContent.ItemType<ClamHermitMedallion>(), Main.expertMode,
                        minibossChance);
                ...
                if (npc.type == calamityMod.NPCType("ThiccWaifu"))
                {
                    if (Main.expertMode)
                    {
                        ConditionalChanceDropItem(npc, ModContent.ItemType<FogG>(),
                            (bool) calamityMod.Call("GetBossDowned", "supremecalamitas"), 0.0001f);
                    ...
                    CalamityWorld.DoGSecondStageCountdown <= 0)
                {
                    ...
                    ChanceDropItem(npc,
                        Main.rand.NextBool() ? ModContent.ItemType<ShellScrap>() : ModContent.ItemType<WeaverFlesh>(),
                        bossPetChance);
