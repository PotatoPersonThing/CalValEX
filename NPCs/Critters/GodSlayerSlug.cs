...
namespace CalValEX.NPCs.Critters
{
    ...
    /// </summary>
    internal class GodSlayerSlug : ModNPC
    {
        ...
        public override void AI()
        {
            if (npc.position.Y > Main.worldSurface * 7.0)
            {
                ...
                npc.active = false;
                npc.netUpdate = true;
                Projectile.NewProjectile(npc.position.X, npc.position.Y, 0f, 7f, 12, 0, 0f, Main.myPlayer);
            }
            else
            {
                return;
        ...
        public override float SpawnChance(NPCSpawnInfo spawnInfo)
        {
            ...
            if ((bool)calamityMod.Call("GetBossDowned", "devourerofgods") && !CalValEXConfig.Instance.CritterSpawns && spawnInfo.player.ZoneSkyHeight)
            {
                if (Main.raining)
                {
                    return 0.4f;
