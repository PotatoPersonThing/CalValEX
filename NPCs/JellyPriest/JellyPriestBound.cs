...
namespace CalValEX.NPCs.JellyPriest
{
    public class JellyPriestBound : ModNPC
    {
        ...
        public override float SpawnChance(NPCSpawnInfo spawnInfo)
        {
            ...
            else if ((bool)clamMod.Call("GetInZone", Main.player[Main.myPlayer], "sulphursea") && (bool)clamMod.Call("GetBossDowned", "acidrain") && !CalValEXWorld.rescuedjelly && !NPC.AnyNPCs(ModContent.NPCType<JellyPriestBound>()) && !NPC.AnyNPCs(ModContent.NPCType<JellyPriestNPC>()) && !CalValEXConfig.Instance.TownNPC)
            {
                return 0.5f;
            }
            else
