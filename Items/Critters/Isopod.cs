...
namespace CalValEX.Items.Critters
{
	...
	internal class Isopod : ModNPC
	{
		...
		public override float SpawnChance(NPCSpawnInfo spawnInfo) 
        {
            ...
            if (clamMod != null)
            {
            if ((bool)clamMod.Call("GetInZone", Main.player[Main.myPlayer], "layer4") && !CalValEXConfig.Instance.CritterSpawns)
                {
                    if (spawnInfo.water)
                    {
        ...
        public override void HitEffect(int hitDirection, double damage) {
			if (npc.life <= 0) {
				Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/Isopod"), 1f);
                Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/Isopod2"), 1f);
                Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/Isopod3"), 1f);
                Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/Isopod4"), 1f);
				}
			}
