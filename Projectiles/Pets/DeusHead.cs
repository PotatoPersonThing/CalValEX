...
namespace CalValEX.Projectiles.Pets
{
    public class DeusHead : ModProjectile
    {
        ...
        public override void AI()
        {
            if (!SpawnedSegments)
            {
                int tail = Projectile.NewProjectile(projectile.Center, Vector2.Zero, ModContent.ProjectileType<DeusTail>(),
                ...
                for (int i = 0; i < SegmentCount - 2; i++)
                {
                    ...
                    int body = Projectile.NewProjectile(projectile.Center, Vector2.Zero,
                        ModContent.ProjectileType<DeusBody>(), projectile.damage, projectile.knockBack,
                        0, uuid);
            ...
            if (player.dead)
            {
                modPlayer.deusmain = false;
            }
            if (modPlayer.deusmain)
            {
        ...
        {
            Texture2D texture = ModContent.GetTexture("CalValEX/Projectiles/Pets/DeusHead_Glow");
