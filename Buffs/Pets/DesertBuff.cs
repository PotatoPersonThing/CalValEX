...
namespace CalValEX.Buffs.Pets
{
    public class DesertBuff : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            {
                Projectile.NewProjectile(player.GetProjectileSource_Buff(buffIndex), player.position.X + (float)(player.width / 2), player.position.Y + (float)(player.height / 2), 0f, 0f, ProjectileType<Projectiles.Pets.DesertPet>(), 0, 0f, player.whoAmI, 0f, 0f);
            }
        }
    }
}