...
namespace CalValEX.Buffs.Pets.Scuttlers
{
    public class EmeraldBuff : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            {
                Projectile.NewProjectile(player.GetProjectileSource_Buff(buffIndex), player.position.X + (float)(player.width / 2), player.position.Y + (float)(player.height / 2), 0f, 0f, ModContent.ProjectileType<Projectiles.Pets.Scuttlers.EmeraldPet>(), 0, 0f, player.whoAmI, 0f, 0f);
            }
        }
    }
}