...
namespace CalValEX.Buffs.PetComboBuffs
{
    public class MaladyBellsBuff : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            ModPlayer TrisPetPlayer = TrisPetMod.GetPlayer("PetPlayer");
            if (TrisPetMod != null)
            {
                bool petProjectileNotSpawnedH = player.ownedProjectileCounts[TrisPetMod.ProjectileType("BloodEel_Projectile")] <= 0;
                if (petProjectileNotSpawnedH && player.whoAmI == Main.myPlayer)
                {
                    Projectile.NewProjectile(player.position.X + player.width / 2 - 64, player.position.Y + player.height / 2,
                        0f, 0f, TrisPetMod.ProjectileType("BloodEel_Projectile"), 0, 0f, player.whoAmI);
                }

                PET.PetPlayer.dreadnautilus = true;
                bool petProjectileNotSpawnedI = player.ownedProjectileCounts[TrisPetMod.ProjectileType("Dreadnautilus_Projectile")] <= 0;
                if (petProjectileNotSpawnedI && player.whoAmI == Main.myPlayer)
                {
                    Projectile.NewProjectile(player.position.X + player.width / 2 - 64, player.position.Y + player.height / 2,
                        0f, 0f, TrisPetMod.ProjectileType("Dreadnautilus_Projectile"), 0, 0f, player.whoAmI);
                }
            }*/
        }
    }
